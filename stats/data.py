...
def generate():
    ...
    print("Récupération des villes françaises les plus peuplées...")
    cities = get_french_cities()

    if not cities:
        print("Impossible de récupérer les données des villes.")
        return

    print(f"Nombre total de villes récupérées : {len(cities)}")

    # Étape 2: Sélectionner aléatoirement n villes parmi les 15 000
    print(f"Sélection de {DESIRED_CITIES} villes aléatoirement...")
    ...
    print(f"Villes sélectionnées : {len(selected_cities)}")
    print(selected_cities[0])
