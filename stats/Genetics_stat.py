...
from Data_reader import data_reader  # Assurez-vous que le chemin est correct
...
def read_cities_from_datareader(filename: str) -> List[City]:
    ...
    try:
        ...
        # Trier les villes par ordre de lecture
        city_names = list(Demands.keys())
        for idx, city_name in enumerate(city_names):
            try:
                demand = Demands[city_name]
                lat, lon = coordinates[city_name]
                cities.append(City(idx + 1, city_name, lat, lon, demand))
            except KeyError:
                print(f"Coordonnées manquantes pour la ville : {city_name}")
        return cities
...
def selection(population: List[List[int]], fitnesses: List[float], num_selected: int) -> List[List[int]]:
    ...
    for _ in range(num_selected):
        if pop_size >= tournament_size:
            ...
            participants = population_with_fitness
...
def genetic_algorithm_tsp(cities: List[City], depot: City, generations=1000, population_size=500, mutation_rate=0.4) -> Dict:
    if not cities:
        print("Aucune ville à traiter dans ce cluster.")
        return None
    ...
    for generation in range(generations):
        ...
        while len(new_population) < population_size:
            if len(selected) >= 2:
                ...
                break  # Pas assez d'individus pour continuer
            child = mutate(child, mutation_rate)
            new_population.append(child)
        population = new_population

    if best_individual is None:
        print("Impossible de trouver une solution pour ce cluster.")
        return None
...
def print_solution(depot: City, result: Dict, truck_number: int, vehicle_capacity: int):
    if result is None:
        print(f"Aucune solution trouvée pour le Camion {truck_number}.")
        return
    route = result['route']
...
if __name__ == "__main__":
    try:
        ...
        # Nombre total de groupes (10 instances de 100 villes)
        num_groups = 10
        ...
        for i in range(num_groups):
            ...
            data_file_path = 'C:/Users/Utilisateur/Arthur/Cours/A3/Algorithme et optimisation combinatoire/Projet/Théophile/Data/data_100.csv'  # Changement du fichier de données
            all_cities = read_cities_from_datareader(data_file_path)

            # Afficher le nombre total de villes chargées
            print(f"Nombre total de villes chargées : {len(all_cities)}")
            ...
            group_cities = all_cities

            # Vérifier si nous avons atteint la fin de la liste
            if not group_cities:
                print(f"Aucune ville dans le groupe {i + 1}.")
                continue
            ...
            # Effectuer le clustering capacitaire pour répartir les villes entre les camions
            clusters = kmeans_capacitated_clustering(group_cities, vehicle_capacity)
            ...
            for j, cluster in enumerate(clusters):
                ...
                if not cluster:
                    print(f"Aucune ville assignée au Camion {j + 1} du groupe {i + 1}.")
                    continue
                ...
                result = genetic_algorithm_tsp(cluster, depot, generations, population_size, mutation_rate)

                # Calculer le temps écoulé
                tf = time.time()
                tt = tf - t0  # Temps total pour résoudre

                # Si aucune solution n'a été trouvée, passer au camion suivant
                if result is None:
                    continue

                # Préparer les données pour Writer_JSON
                nom_ville_cluster = [city.name for city in cluster]
                Nom_Algo = "Genetics : generations, " + str(generations) + " ; population size, " + str(population_size) + " ; mutation rate, " + str(mutation_rate)

                Nombre_Camion = j + 1  # Numéro du camion
                ...
                JSON_Path = 'C:/Users/Utilisateur/Arthur/Cours/A3/Algorithme et optimisation combinatoire/Projet/Théophile/result_genetics.json'  # Chemin vers votre fichier result.json

                # Utiliser la fonction Writer_JSON modifiée
                STAT_JSON(
                ...
                # Afficher la solution pour le camion
                print_solution(depot, result, j + 1, vehicle_capacity)
